...
import { clone, hasMethod, isFunction, excludeSpecificTasks, utilClearByTag } from './utils';
...
let Queue = (() => {
  ...
  function _constructor(config) {
    ...
    this.storage = new StorageCapsule(this.config, new LocalStorage(this.config));
  ...
  function loopHandler() {
    const self = this;
    const task = db.call(self).fetch().shift();
    ...
    // Task runner promise
    jobInstance.handle.call(jobInstance, task.args, ...dependencies).
  ...
  function fireJobInlineEvent(name, job, args) {
    ...
    if (name == 'after' && isFunction(job.after)) {
