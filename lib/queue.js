...
Queue.prototype.clearByTag = (() => {
  const _ref7 = _asyncToGenerator(function* (tag) {
    const _this = this;
    (yield db.call(this).all()).filter(utilClearByTag.bind(tag)).forEach(t => db.call(_this).delete(t._id));
  });
 ...
 */
Queue.prototype.has = (() => {
  const _ref8 = _asyncToGenerator(function* (id) {
    return (
...
Queue.prototype.hasByTag = (() => {
  const _ref9 = _asyncToGenerator(function* (tag) {
    return (
      (yield getTasksWithoutFreezed.call(this)).findIndex(t => t.tag === tag) > -1
    );
  });
  function hasByTag(_x5) {
 ...
 * @api public
 */
Queue.prototype.setNetwork = function setNetwork(val) {
 ...
 * @api public
 */
Queue.register = function register(jobs) {
